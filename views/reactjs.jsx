...
// // const mongoose = window.mongoose;//require('mongoose')
...
//     return (
//         <>
//         <h1> this is the page we come through</h1>
//         </>
//     );
// }

// vewkv=-ewbvmpe-0wvmekmbl-=fqwfmwqf=fqwfmwqf
// fkwqf-0kfg=-vbmdme=-qow=f
// lfv[pwqk0-fkwmgegv=wego=gvoew=fgwemgopvmegw-kv=-ew]
// mfwq-0fj-0qwkf-=<efkopmvew-jv-0ewvj-0ekvpewvm></efkopmvew-jv-0ewvj-0ekvpewvm>
// kfqwmfowq-f0mocmoimv-0qwkf-=
// oqf0qjwf89hbfiwqbfwqnfwqf90wfjiwqfnwnfiowfjiwqjfwqf=--0fansofnasifjasjfsajfposafjkopsk
// mfkansiofsafjjfsf[spfls[af-=wof=w-ofwlfs,fcmsxmcvsf-=f-=wofkwfsm,flsamfpsamf-0wkfmsalmflsmf-ksafplsa]
// f,alpsmfo0asjfo0safpamsfopmsaofsaokf]f;<div psfkosajfosafonsofnsioafosakfklsnfksofosokfoskflsfskfposopfksopakfopskfopsakfopksaopfksopkfsafsa[lf[psa]]="
// "></div>
...
// As of 22 June 2005, Gmail's canonical URI changed from http://gmail.google.com/gmail/ to http://mail.google.com/mail/.[17] As of November 2015, those who typed in the former URI were redirected to the latter.

// On 2 November 2006, Google began offering a mobile-application based version of Gmail for mobile phones capable of running Java applications. In addition, Sprint announced separately that it would make the application available from its Vision and Power Vision homepages, preloaded onto some new Sprint phones. The application gives Gmail its own custom menu system and the site displays attachments, such as photos and documents in the application.[18][19]

// On 28 January 2007, Google Docs & Spreadsheets was integrated with Gmail, providing the capability to open attached Microsoft Word DOC files directly from Gmail.[20]
...
// On 24 October 2007, Google announced that IMAP was available for all accounts, including Google Apps for your Domain.[21]

// On 5 June 2008, Google introduced Gmail Labs.[22]

// On 8 December 2008, Google added a to-do list to Gmail. When the new Tasks feature is enabled, a box shows up on top of the Gmail window. In it, users can add, reorder and delete tasks. It is also possible to assign a due date to each action and even convert e-mails into tasks.[23]

// On 12 December 2008, Gmail added support for PDF viewing within the browser.[24]

// On 24 February 2009, Gmail suffered a two and a half hour outage, affecting 100 million accounts.[25]

// On 7 July 2009, Gmail officially exited its beta status in a move to attract more business use of the service.[26][27]

// On 1 September 2009, Gmail suffered another outage for several hours.[28]
...
// On 4 July 2005 Google announced that Gmail Deutschland would be rebranded as Google Mail. The domain gmail.com became unavailable in Germany due to trademark disputes, in which cases users must use the domain googlemail.com.[29] From that point forward, visitors originating from an IP address determined to be in Germany would be forwarded to googlemail.com where they could obtain an email address containing the new domain.[30]

// The domains are interchangeable so users obliged to use the googlemail.com domain are unable to select addresses already chosen by gmail.com users.[30] Inbound emails sent to either googlemail.com or gmail.com addresses will reach the user.

// The German naming issue is due to a trademark dispute between Google and Daniel Giersch, who owns a German company called "G-mail" which provides the service of printing out email from senders and sending the print-out via postal mail to the intended recipients. On 30 January 2007, the EU's Office for Harmonization in the Internal Market ruled in favour of Giersch.[31]

// Google spoofed "offering" the same service in the Gmail Paper April Fool's Day joke in 2007.[32]

// On 13 April 2012, Google received the right to the Gmail trademark in Germany. On that day the gmail.de domain and the Gmail trademark were transferred to Google.[33]

// Poland
// In February 2007 Google filed legal action against the owners of gmail.pl, a poet group known in full as Grupa Młodych Artystów i Literatów abbreviated GMAiL (literally, "Group of Young Artists and Writers").[34] This lawsuit was lost, but the Web site no longer exists.[35]

// Russian Federation
// A Russian paid mail redirect service called gmail.ru owns the "Gmail" trademark in the Russian Federation.[36]

// The gmail.ru domain name dates from 27 January 2003.[37] The gmail.ru domain name was auctioned.[38]

// United Kingdom
// On 19 October 2005, Google voluntarily converted the United Kingdom version of Gmail to Google Mail because of a dispute with the UK company Independent International Investment Research.[39][40]
...
// In September 2009 Google began to change the branding of UK accounts back to Gmail, following the resolution of the trademark dispute.[41]

// On 3 May 2010, Google announced that they would start to phase out the googlemail.com domain in the UK. Existing users would get the option to switch to gmail.com, while new users would be given a gmail.com address by default.[42] This also required Android phone users to perform a factory reset (requiring a back-up to prevent data loss) to restore phone functionality.[43]
...
// References
//  "Welcome to Official Gmail Blog". 3 July 2007.
//  "Paul Buchheit on Gmail, AdSense and More". Google Blogoscoped. 25 January 2007. Retrieved 23 November 2013. (quoting from: Jessica Livingston, Founders at Work, ISBN 978-1590597149)
//  McCracken, Harry (1 April 2014). "How Gmail Happened: The Inside Story of Its Launch 10 Years Ago". Time. Retrieved 19 April 2014.
//  Sullivan, Danny (30 March 2004). "Google Launches Gmail, Free Email Service". SearchEngineWatch.com. Retrieved 12 March 2008.
//  Harry McCracken (1 April 2013). "Google's Greatest April Fools' Hoax Ever (Hint: It Wasn't a Hoax)". TIME.com. Archived from the original on 1 April 2013. Retrieved 1 August 2014.
//  Lisa Baertlein (1 April 2004). "Google: 'Gmail' no joke, but lunar jobs are". Reuters. Retrieved 1 August 2014.
//  Google (1 April 2004). "Google Gets the Message, Launches Gmail". Retrieved 1 June 2008. {{cite web}}: |author= has generic name (help)
//  Google (1 April 2002). "PigeonRank: "The technology behind Google's great results"". Retrieved 1 June 2008. {{cite web}}: |author= has generic name (help)
//  Google (1 April 2004). "Google Copernicus Center is hiring". Archived from the original on 8 June 2012. Retrieved 1 June 2008. {{cite web}}: |author= has generic name (help)
//  BBC News Online (1 April 2004). "Timing makes Google an April Fool". Retrieved 1 June 2008.
//  The Google Story, pg 154, yr. 2005, David A. Vise, Mark Malseed
//  Ulbrich, Chris. "Gmail Invitation Prices Crash." Wired. June 10, 2004. Retrieved on November 7, 2009.
//  Hailey, Arin (26 May 2004). "Gmail Hits the Auction Block". PC World. Archived from the original on 4 June 2004. Retrieved 29 September 2018.
//  Google (September 2008). "Gmail Program Policies". Retrieved 24 November 2008. {{cite web}}: |last= has generic name (help)
//  Wally, Michael (January 2005). "Gmail Messages Are Vulnerable To Interception". Retrieved 24 November 2008.
//  "Gmail Invite Spooler Post-Mortem". 23 March 2006.
//  Mathias Bynens (25 June 2005). "Google goes 301". Archived from the original on 11 August 2007. Retrieved 25 November 2007.
//  Oswald, Ed (November 2006). "Google Offers Java-based Mobile Gmail". Retrieved 2 November 2006.
